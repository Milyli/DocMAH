using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace DocMAH.Web.Requests
{
	public static class RequestTypes
	{
		#region Public Fields

		public const string Css = "CSS";
		public const string DeletePage = "DeletePage";
		public const string DocumentationPage = "DocumentationPage";
		public const string GenerateInstallScript = "GenerateInstallScript";
		public const string JavaScript = "JavaScript";
		public const string MovePage = "MovePage";
		public const string NotFound = "NotFound";
		public const string ReadApplicationSettings = "ReadApplicationSettings";
		public const string ReadPage = "ReadPage";
		public const string ReadTableOfContents = "ReadTableOfContents";
		public const string SavePage = "SavePage";
		public const string SaveUserPageSettings = "SaveUserPageSettings";
		public const string Unauthorized = "Unauthorized";

		#endregion
	}
}
